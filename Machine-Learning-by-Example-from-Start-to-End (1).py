...
# ***Markdown Cell***
# 1.  ![Workflow Diagram](images/classification/workflow.jpg)
# 
# [Workflow Diagram](https://www.researchgate.net/figure/The-flowchart-of-the-prediction-model_fig1_358222578/actions#reference)
# 
# Development of a Web-Based Prediction System for Students’ Academic Performance - Scientific Figure on ResearchGate. Available from: https://www.researchgate.net/figure/The-flowchart-of-the-prediction-model_fig1_358222578 [accessed 02 Dec, 2023]
# 
# 
# 2. In relation to the lectures, the decision tree example aligns with the workflow as it involves a training dataset. The example in the lectures involves tv show candidates which are then organised in categories, through yes or no questions, of whether the candidates do or do not have a computing degree. This data is then used to create a decision tree which splits these candidates and results in three leaf nodes which is the algorithm. In this example there is no testing data or evalution provided however we are aware that the algorithm is overfitting.
# 
# 3. It is important when relating to the audience, to provide examples that are simplistic, ensure clarity and are somewhat relatable. For a museum curator, it would be helpful to use art exhibitions as an example as the process begins with collecting data such as various artworks. The algorithm in this case would be curatorial expertise; being able to recognise artistic styles and historical contexts. The model would then be the curator's mental framework where they categorise and arrange exhibits based on their understanding. 
# 
# 4. In my peer group we discussed how adding comments is essential for an audience and additionally for yourself to keep track of progress and maintain an understanding. This is because it provides clarity and helps keep information managed and structures which is crucial for ensuring that code can be read clearly. 
...
# 
# Step 1. discussion: framing your problem precisely is very important for defining clear objectives, selecting appropriate alogirtms and is also significant as same data can be a valuable resource, but may be problematic if not managed responsibly. 
# 
# Steps 2&3;
# 
# 1. In discussion with my peer group; we concluded that for predicting median house pricing, regression is a more suitable choice as the model is designed to predict continuous values which in this case would be estimating housing prices of various numeric quantities. 
# 
# 2. For handwritten digit recognition, we noted that classification would be a better fit as the model is designed for tasks that assign various inputs into pre-defined categories, which in this case is suitable as handwritten digit recognition typically classifies each image into discrete categories representing the digits. 
# 
# 3. For predicting the median housing price we discussed that the factors in the housing district that should be considered consist of; location, property size, and number of rooms and bathrooms.
# 
# 4. Predicting median housing prices must consider geographical and cultural variations in preferences, as data collection may introduce biases that stem from differences in regional and cultural priorities.
# 
# 
...
# 
# 1. From our group discussions we established that each image size is 28x28
# 2. They collected the SD-3 (the training data set used by the NIST original dataset) dataset amongst Census Bureau employees, which was easier to recognise. Additionally, the SD-1 (the test dataset used by the NIST original dataset) was collected by highschool students. MNIST was then created by mixing both datasets. 
# 3. We believe they did this to draw better conclusions 
# 4. As they are training the model with a wider range of handwritten patterns, i think that this methods was the right choice as it overall lessens the bias, making it potentially more accurate. 
...
print ("Assigned Categories:", categories)
...
# ***Markdown cell***
# 
# 1. A stratified sample based on median income is reasonable as it ensures representation from various income brackets within the population which then prevents the underrepresentation of specific income groups. 
# 2. We also discussed how a stratified sample helps capture the the economic diversity of the population, which provides as more well rounded understanding about how different income levels can show an impact. 
# 3. A member in my group also noted how it allows for targeted analysis of subgroups such as comparing outcomes and behaviours of different incomes. 
# 4. Lastly, we noted how it is reasonable for ensuring that findings and conclusions are not skewed by overemphasis on any particular income range, therefore providing a more balanced interpretation. 
# 
...
# 
# 1. In this scenario i will create a model to predict user satisfaction with a mobile app.
#    Collecting my own data: i will collect survey responses from users about their app experience, covering areas such as; the user interface, app features and overall satisfaction. Additionally, i will also capture screenshots of different app screens. In my code, i will then integrate survey responses and screenshots to ensure proper alignment to create a comprehensive dataset.
#    
#    Changes to Model: I would change my model from a basic decision tree model to a random forest model to capture the complexity of user pereferences better. I would adjust my code to implement the random forest algorithm, modifying parameters and adapting input features as needed. I would also change my scaling method from Min-Max scaling to Z-score scaling for better convergence in the random forest model, update the code to apply Z-score scaling to relevant features in both the survey data and screenshots. 
#    
#    My approach for handling missing data: I would refine my approach to handle missing data by incorporating mean imputation for survey responses and use image impainting techniques for missing parts in the screenshots. I would adjust my code to include these new strategies during the data preprocessing phase. 
#    
#    The significance of cross validation: I would ensure the code incorporates k-fold cross-validation when evaluating the performance of the updated model. Cross validation is significant because it aids in obtaining a more robust assessment of the model's ability to generalise to (in this case) new app users. 
# 
# 
# 2. Tensorflow: i was able to get the training loss down to 0.124 by experimenting through various different options. i experimented through using adding and removing different features, including hidden features. I realised that using the x^ features generally increased the training loss so i took those out. After experimenting with many hidden features i ended up removing them as they were unsuccessful. It was very difficult to determine what functions were beneficial. 
# 3.  ![Screenshot](images/classification/tensorflow.png)
# 
